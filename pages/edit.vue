...
export default {
  ...
  validate({ query }) {
    if (query.id === undefined) return false;
    return true;
  },
  async asyncData({ app, error, query, store }) {
    const id = query.id;
    store.commit('resetInput');
    await store.dispatch('fetchItem', id);
  ...
  methods: {
    async register() {
      await this.$store.commit('updateSelectedUsers', this.id);
      this.$router.push('/');
    }
  }
}
